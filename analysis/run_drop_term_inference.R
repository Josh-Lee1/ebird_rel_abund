...
OUT_FILE <- "results_500m_inference/drop_term_inference_results.csv"
if (file.exists(OUT_FILE)) {
  ...
  out_cols <- c("species", "dev_full", "dev_base", "dev_no_fd", "dev_no_cwm", "dev_no_rich", "dev_no_struct", "unique_total_veg", "unique_fd", "unique_cwm", "unique_richness", "unique_structure")
...
if (length(run_species) == 0) {
  cat("All species completed.\n")
  quit(save = "no")
}

cat("Running Drop-Term CV Analysis for", length(run_species), "species...\n")
...
process_species_cv <- function(sp) {
  ...
  fd_cols_valid <- valid_smooths(fd_vars)
  cwm_cols_valid <- valid_smooths(cwm_vars)
  rich_cols_valid <- valid_smooths(rich_vars)
  struct_cols_valid <- valid_smooths(struct_vars)
  
  all_veg_valid <- c(fd_cols_valid, cwm_cols_valid, rich_cols_valid, struct_cols_valid)
  
  form_full      <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = all_veg_valid)
  form_base      <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = NULL)
  form_no_fd     <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = setdiff(all_veg_valid, fd_cols_valid))
  form_no_cwm    <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = setdiff(all_veg_valid, cwm_cols_valid))
  form_no_rich   <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = setdiff(all_veg_valid, rich_cols_valid))
  form_no_struct <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = setdiff(all_veg_valid, struct_cols_valid))
  ...
  tryCatch({
    ...
    cv_base      <- evaluate_model_cv(df_samp, formula = form_base, fold_ids = my_folds, gamma = 1.0)
    cv_no_fd     <- evaluate_model_cv(df_samp, formula = form_no_fd, fold_ids = my_folds, gamma = 1.0)
    cv_no_cwm    <- evaluate_model_cv(df_samp, formula = form_no_cwm, fold_ids = my_folds, gamma = 1.0)
    cv_no_rich   <- evaluate_model_cv(df_samp, formula = form_no_rich, fold_ids = my_folds, gamma = 1.0)
    cv_no_struct <- evaluate_model_cv(df_samp, formula = form_no_struct, fold_ids = my_folds, gamma = 1.0)
    
    res <- data.frame(
      species = sp,
      dev_full = cv_full$summary["holdout_dev_expl"],
      dev_base = cv_base$summary["holdout_dev_expl"],
      dev_no_fd = cv_no_fd$summary["holdout_dev_expl"],
      dev_no_cwm = cv_no_cwm$summary["holdout_dev_expl"],
      dev_no_rich = cv_no_rich$summary["holdout_dev_expl"],
      dev_no_struct = cv_no_struct$summary["holdout_dev_expl"]
    )
    
    res$unique_total_veg <- res$dev_full - res$dev_base
    res$unique_fd <- res$dev_full - res$dev_no_fd
    res$unique_cwm <- res$dev_full - res$dev_no_cwm
    res$unique_richness <- res$dev_full - res$dev_no_rich
    res$unique_structure <- res$dev_full - res$dev_no_struct
