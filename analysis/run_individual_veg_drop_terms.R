...
# To ensure this doesn't crash your 16GB machine overnight, we strictly 
# limit this to 6 parallel workers. This balances speed and memory safety.
# ==============================================================================
SAFE_CORES <- 6
...
OUT_FILE <- "results_500m_inference/individual_veg_drop_results.csv"

# 12 variables we want to isolate
all_veg_vars <- c("FRic", "FEve", "FDiv", 
                  "wooddensity_cwm", "LA_cwm", "height_cwm", 
                  "sp_richness", "pielou_evenness", "gini_simpson", 
                  "cover", "mean_fhd", "prop_canopy")

# Generate output column names
out_cols <- c("species", "dev_full")
for (v in all_veg_vars) {
  out_cols <- c(out_cols, paste0("dev_no_", v), paste0("unique_", v))
}
...
cat("Running Individual Veg Drop-Term CV Analysis for", length(run_species), "species...\n")
cat("Using", SAFE_CORES, "cores for RAM safety.\n")
...
process_species_cv <- function(sp) {
  ...
  all_veg_valid <- valid_smooths(all_veg_vars)
  ...
  tryCatch({
    ...
    dev_full_val <- cv_full$summary["holdout_dev_expl"]
    
    # 2. Setup results row
    res <- data.frame(species = sp, dev_full = dev_full_val)
    
    # 3. Fit 12 drop models
    for (v in all_veg_vars) {
      if (v %in% all_veg_valid) {
        form_drop <- build_gam_formula(df_samp, hab_cols, extra_smooth_cols = setdiff(all_veg_valid, v))
        cv_drop <- evaluate_model_cv(df_samp, formula = form_drop, fold_ids = my_folds, gamma = 1.0)
        
        dev_no_v <- cv_drop$summary["holdout_dev_expl"]
        res[[paste0("dev_no_", v)]] <- dev_no_v
        res[[paste0("unique_", v)]] <- dev_full_val - dev_no_v
      } else {
        # Variable wasn't valid for this species (e.g. no variance)
        res[[paste0("dev_no_", v)]] <- NA
        res[[paste0("unique_", v)]] <- NA
      }
    }
...
message(sprintf("Starting parallel processing across %d cores...", SAFE_CORES))
...
clusterExport(cl, c(
  ...
  "all_veg_vars", "process_species_cv"
))
...
for (res in results) {
  if (res$success) {
    ...
    res_ordered <- res$res[, out_cols]
    write.table(res_ordered, OUT_FILE, sep = ",", col.names = FALSE, row.names = FALSE, append = TRUE, quote = TRUE)
    n_ok <- n_ok + 1
